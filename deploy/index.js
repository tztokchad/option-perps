module.exports = async hre => {
  ...
  const quoteLpPositionMinter = await deployments.deploy("LpPositionMinter", {
    args: ["USDC", "DOPEX-USDC-O-P-LP", 6],
    from: deployer,
    log: true,
  });

  const baseLpPositionMinter = await deployments.deploy("LpPositionMinter", {
    args: ["ETH", "DOPEX-ETH-O-P-LP", 18],
    from: deployer,
    log: true,
  });

  const perpPositionMinter = await deployments.deploy("PerpPositionMinter", {
    args: [],
    from: deployer,
    log: true,
  });

  const optionPositionMinter = await deployments.deploy("OptionPositionMinter", {
    args: [],
    from: deployer,
    log: true,
  });
  ...
  const _quoteLpPositionMinter = await hre.ethers.getContractAt(
    abi,
    quoteLpPositionMinter.address,
    deployer
  );

  const _baseLpPositionMinter = await hre.ethers.getContractAt(
    abi,
    baseLpPositionMinter.address,
    deployer
  );

  const _perpPositionMinter = await hre.ethers.getContractAt(
    abi,
    perpPositionMinter.address,
    deployer
  );

  const _optionPositionMinter = await hre.ethers.getContractAt(
    abi,
  ...
  await _perpPositionMinter.setOptionPerpContract(optionPerp.address);
  await _optionPositionMinter.setOptionPerpContract(optionPerp.address);
};
